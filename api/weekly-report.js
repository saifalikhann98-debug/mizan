...
module.exports = async (req, res) => {
  ...
  // deep-link only services buckets (rent/motor use different pickers); us- prefixed ids live under /us/
  const seed = close.filter(o => !/^(us-)?(motor-|rent-)/.test(o.c)).map(o =>
    `<div style="font-size:13px;margin:4px 0"><a href="https://www.mizan-price.com/${o.c.startsWith('us-') ? 'us/' : ''}?service=${encodeURIComponent(o.c)}&amp;area=${encodeURIComponent(o.a)}" style="color:#173F35;font-weight:600">${esc(o.c)} @ ${esc(o.a)}</a> — needs ${5 - o.n} more to flip</div>`).join('');
