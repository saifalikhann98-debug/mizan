...
module.exports = async (req, res) => {
  ...
  const row = o => `<tr><td style="padding:4px 10px 4px 0;font-weight:700;color:#14534D;white-space:nowrap">${o.n}/5</td><td style="padding:4px 12px 4px 0">${esc(o.c)} <span style="color:#8A857B">@ ${esc(o.a)}</span></td><td style="padding:4px 0;color:#55514A;white-space:nowrap">~AED ${o.med}</td></tr>`;
  ...
  const seed = close.filter(o => !o.c.startsWith('motor-')).map(o =>
    `<div style="font-size:13px;margin:4px 0"><a href="https://www.mizan-price.com/?service=${encodeURIComponent(o.c)}&amp;area=${encodeURIComponent(o.a)}" style="color:#14534D;font-weight:600">${esc(o.c)} @ ${esc(o.a)}</a> — needs ${5 - o.n} more to flip</div>`).join('');
